#ifndef PATL_IMPL_CONST_VERTEX_HPP
#define PATL_IMPL_CONST_VERTEX_HPP
...
protected:
    ...
    typedef typename cont_type::const_levelorder_iterator const_levelorder_iterator;
...
public:
    ...
    prefix get_prefix() const // prefix тоже должен быть const/non-const
    {
    ...
    const_preorder_iterator preorder_begin() const
    {
        ...
        return const_preorder_iterator(this_t(pal_.cont(), q, q ? get_qid() : 1));
    }
    ...
    const_levelorder_iterator levelorder_begin(word_t limit) const
    {
        this_t vtx(*this);
        if (compact())
            vtx.descend<0>(limit);
            ...
            vtx.toggle();
        return const_levelorder_iterator(limit, vtx);
    }

    const_levelorder_iterator levelorder_end(word_t limit) const
    {
        if (pal_.get_q())
            return ++const_levelorder_iterator(limit, *this);
        else
        {
            this_t vtx(*this);
            vtx.toggle();
